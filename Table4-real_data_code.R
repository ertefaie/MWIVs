...
#dat<-dat[dat$r_sex=="Male"&as.numeric(dat$age_cat_ssd)>4,]
...
#temp<-aggregate(r_county_new~r_judge_id,data=temp,FUN=mean)
#temp<-temp[order(temp[,2]),]
...
# delta=0, K gives -0.90 with %95 CI (-1.15,-0.65)
# 2SLS gives -1.22 with %95 CI (-1.22-2*0.11,-1.22+2*0.11)
# lm gives -1.27 with %95 CI (-1.27-2*0.02,-1.27+2*0.02)
...
#example <- as.data.frame(dat$pracid)

#names(example) <- "pracid"


#save(list=ls(),file="Subgroup_analyses2.RData")

#For every unique value in the string column, create a new 1/0 column

#This is what Factors do "under-the-hood" automatically when passed to function requiring numeric data

#for(level in unique(example$pracid)){

 # example[paste("dummy", level, sep = "_")] <- ifelse(example$pracid == level, 1, 0)
