...
#Specify a grid for beta1 and beta2
temp1<- seq(-.2,.2,by=0.01)
temp2<- seq(-.2,.2,by=0.01)
...
em.mK32<-matrix(em.power.KJ32, nrow=length(temp1),ncol=length(temp2),byrow=TRUE)
em.mK14<-matrix(em.power.KJ14, nrow=length(temp1),ncol=length(temp2),byrow=TRUE)
...
contour(PP$beta1, PP$beta2, round(PP$KJ32,2),add=TRUE,lty=2)
